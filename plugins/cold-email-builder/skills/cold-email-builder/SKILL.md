...
**⛔ The stored `ask` is a HINT, not the email's CTA verbatim (added 31 Aug 2026 after cross-skill testing).** `ask` is a shared brain field, and `sales-brain-setup` writes it as an internal *next-Bingley-tool* recommendation ("build a prospect list", "write/fix your cold email", "research the account before your next call"), not a reader-facing call-to-action. So before using stored `ask` as the email's CTA: if it reads like a Bingley/next-tool action or an instruction to the USER rather than a request TO the recipient (book a call, reply, grab a slot), IGNORE it and derive the CTA from the offer at the right friction for the profile — or, if genuinely unsure, ask the one-line CTA question. A real email must never carry "write/fix your cold email" (or similar) as its call-to-action.

1. From the stored profile plus whatever the user gave at the door (Entry section), draft to the rubric's "5 looks like" anchors: them-led opener on a real signal, one concrete problem-moment **stated as fact, not hypothetical**, a single CTA at the right friction for the profile (see the `ask` guard above), profile-appropriate proof, tight and scannable, clean subject. **Open and close like a real email:** open with a greeting to the recipient by name ("Hi [Name],"), and close with a sign-off followed by the sender's name on its own line, with nothing after the name. A bare name is enough (it need not say "Kind regards"), but on a cold email there must always be a sign-off. Any sender bio or credibility line goes in the body, before the sign-off, never after it.
